...
class KernelDensitySource(SourceBase):
    ...
    path : str or pathlib.Path
        Path to the source file from which sites should be sampled
    perturb: bool (default is True)
        Variable to choose whether new particles will be generated by
        disturbing the phase space of the original ones.
    adjust_weight: bool (default is True)
        Variable to choose whether new particles are generated 
        with weight 1 or with the original weight. KDsource applies 
        variance reduction methods to not disturb the original distribution
        if the wieght are adjusted to one.
    strength : float
        Strength of the source (default is 1.0)
    ...
    strength : float
        Strength of the source
    type : str
        Indicator of source type: 'KDSource'

    """
    def __init__(self, path: Optional[PathLike] = None, perturb = True, adjust_weight=True , strength=1.0) -> None:
    ...
    @perturb.setter
    def perturb(self,r: bool):
        cv.check_type('perturb',r ,bool)
        self._perturb = r

    @adjust_weight.setter
    def adjust_weight(self, w: bool):
        cv.check_type('adjust_weight', w, bool)
        self._adjust_weight = w
    ...
    def populate_xml_element(self, element):
        ...
        if self.path is not None:
            ...
            element.set("adjust_weight", str(self.adjust_weight))
